...
import type { FileChange, PRContent, PRGeneratorConfig, PRResult } from './types.js';
...
export class PRGenerator {
  ...
  async generatePR(
    ...
    try {
      ...
      const result = await this.github.createPullRequest(branchName, this.baseBranch, prContent);
  ...
  async generatePRs(
    ...
    for (const improvement of improvements) {
      ...
      const result = await this.generatePR(improvement, rulesFilePath, originalContent, newContent);
      results.push(result);
   ...
   * Generate PR content
   */
  private generatePRContent(improvement: RuleImprovement, _changes: FileChange[]): PRContent {
  ...
  private generatePRBody(improvement: RuleImprovement): string {
    ...
    sections.push('');
    sections.push(
      'This PR was automatically generated by **Blackbox CI** based on analysis of recent agent traces.'
    );
    sections.push('');
    ...
    sections.push('---');
    sections.push(
      '*Generated by [Blackbox CI](https://github.com/example/blackbox) - Nightly CI for Coding Agents*'
    );
  ...
  async checkAndAutoMerge(prNumber: number): Promise<boolean> {
    ...
    if (this.config.requiredChecks && this.config.requiredChecks.length > 0) {
      const passedChecks = checks.filter((c) => c.conclusion === 'success').map((c) => c.name);

      const missingChecks = this.config.requiredChecks.filter((rc) => !passedChecks.includes(rc));
